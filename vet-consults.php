...
// Step 2: Database se vet-consults table ka data nikalne ki query run ki
...
<body>
    ...
    <main class="main-content">
        <div class="header">
            <div class="header-text">
                ...
                <p style="color: #64748b;">Manage available doctors, specializations, and scheduling</p>
            </div>
        ...
        <section class="table-card">
            <form action="update-appointment.php" method="POST">
                ...
                <?php
                // Step 3: PHP Loop jo database se doctor records nikalega
                ...
                ?>
                        <div class="form-data-row">
                            ...
                            <div class="row-name">
                                <input type="text" name="doc_name[]" value="<?php echo $row['doc_name']; ?>">
                            </div>
                            <div>
                                <select name="doc_spec[]">
                                    <option <?php if($row['doc_spec'] == 'Surgery') echo 'selected'; ?>>Surgery</option>
                                    <option <?php if($row['doc_spec'] == 'General Physician') echo 'selected'; ?>>General Physician</option>
                                    <option <?php if($row['doc_spec'] == 'Vaccination') echo 'selected'; ?>>Vaccination</option>
                                </select>
                            </div>
                            <div>
                                <input type="text" name="doc_fee[]" value="<?php echo $row['doc_fee']; ?>">
                            </div>
                            <div>
                                <span class="badge"><?php echo $row['doc_status']; ?></span>
                            ...
                            <div class="actions-cell">
                                ...
                                <i class="fas fa-trash delete-icon" onclick="ajaxDeleteRow(this, '<?php echo $row['doc_id']; ?>')"></i>
                            </div>
                        </div>
                <?php
                    }
                    ...
                    echo "<p style='padding: 30px; text-align: center; color: #64748b;'>No doctors found in schedules. Click 'Add New Doctor' to insert data.</p>";
                }
    ...
    function ajaxDeleteRow(element, id) {
        if (confirm("Are you sure you want to delete this doctor?")) {
            // Background mein delete-appointment.php file hit hogi bina page reload kiye
            fetch('delete-appointment.php?id=' + encodeURIComponent(id), {
                method: 'GET'
            })
            .then(response => {
